...
class Net extends EventEmitter {
  ...
  sendPing (target) {
    const message = this.messages.pingMessage(target, this.update.getUpdates())
    this._send(message)
  }

  sendPingReq (target) {
    const message = this.messages.pingReqMessage(target, this.update.getUpdates())
    this._send(message)
  }

  sendAck (target) {
    const message = this.messages.ackMessage(target, this.update.getUpdates())
    this._send(message)
  }

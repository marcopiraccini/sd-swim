...
class Net extends EventEmitter {
  ...
  sendUpdateJoin (destination, members) {
    const message = this.messages.updateJoinMessage(destination, members)
    this._send(message)
  }

  sendPing (destination, target, requester) {
    const message = this.messages.pingMessage(destination, target, requester, this.update.getUpdates())
    this._send(message)
  }

  sendPingReq (destination, target, requester) {
    const message = this.messages.pingReqMessage(destination, target, requester, this.update.getUpdates())
    this._send(message)
  }

  sendAck (destination, target, requester) {
    const message = this.messages.ackMessage(destination, target, requester, this.update.getUpdates())
    this._send(message)
  }

...
class Net extends EventEmitter {
  ...
  sendUpdateJoin (target, members) {
    const message = this.messages.updateJoinMessage(target, members)
    this._send(message)
  }

  sendPing (target, updates) {
    const message = this.messages.pingMessage(target, updates)
    this._send(message)
  }

  sendPingReq (target, updates) {
    const message = this.messages.pingReqMessage(target, updates)
    this._send(message)
  }

  sendAck (target, updates) {
    const message = this.messages.ackMessage(target, updates)
    this._send(message)
  }

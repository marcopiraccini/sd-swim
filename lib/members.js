...
class Members extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.members = [] // The actual member list
  ...
  /**
   * Add member to member list with a node and state and add the updates to be propagated.
  ...
  addOrUpdateMember(node = this.sdswim.me, state = ALIVE,
    ...
    this.emit('updated-members', this.members)
    this.update.addUpdate(node, setBy, state, incNumber)
  }
  ...
  updateToAlive(member) {
    ...
    if (state === SUSPECT) {
      this.update.addUpdate(node, ALIVE, this.sdswim.me, incNumber)
   ...
   * Creates also the related updates
   */
  updateToFaulty(member, incNumber) {
    const removed = remove(this.members, el => (el.node.host === member.host  && el.node.port === member.port))
    this.update.addUpdate(member, this.sdswim.me, FAULTY, incNumber)

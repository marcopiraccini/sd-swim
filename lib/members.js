...
class Members extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.members = [] // The actua member list
  ...
  updateToAlive(node) {
    ...
    if (current.state === SUSPECT) {
      ...
      this.update.addUpdate(node, ALIVE, this.sdswim.me, current.incNumber)
  ...
  updateToSuspect(node) {
    ...
    if (current.state === ALIVE) {
      ...
      current.setBy = this.sdswim.me
      this.update.addUpdate(node, SUSPECT, this.sdswim.me, current.incNumber)

      // Suspect timeout: when triggered it check if the node is still SUSPECT.
      // If so, it's set to faulty.
      setTimeout(() => {
        if (current.STATE === SUSPECT) {
          this.updateToFaulty(current.node, current.incNumber)
        }
      }, this.suspectTimeout)
   ...
   * Creates also the related updates
   */
  updateToFaulty(node, incNumber) {
    const removed = remove(this.members, el => (el.node.host === node.host  && el.node.port === node.port))
    this.update.addUpdate(node, this.sdswim.me, FAULTY, incNumber)

...
class Members extends EventEmitter {
   ...
   */
  removeMember (member) {
    const prevNum = this.members.length
    remove(
      this.members,
      el => el.node.host === member.host && el.node.port === member.port
    )
    if (prevNum !== this.members.length) {
      this.emit('peerDown', member) // emitted only if the member hasn't laready removed
    }
  }

  // Get next `n` members for the ping/ping-req. The members array is traversed
  // circularly  and if traversal is completed in the process, is then shuffled

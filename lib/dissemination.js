...
// - A list of current "updates" to be propagated
// - A list of recent "faulties", which is necessary to avoid that case a node is marked
//   as Faulty (so removed) but there are still "Alive" messages around (so is then readded)
// - A list of "expired" updates, i.e. the updates that has already been gossiped up up
//   to the limit. We have to drop this ones to avoid messages loops.
...
class Dissemination {
  ...
  constructor(opts) {
    ...
    // Faulties / expired clean up. Parametrizied using `this.suspectTimeout`
    setInterval(() => {
      const now = Date.now()
      const newExpired = {}
      const newFaulties = {}
  ...
  addUpdate(node, setBy, state, incNumber, uuid) {
    ...
    if (!update) {
      if (!uuid) {
        this._updates.push({node, setBy, state, incNumber, gossiped: 0, uuid: uuidV4(null, uuid, 0)})
      } else {

...
class Update {
  ...
  addUpdate(node, setBy, state, incNumber) {
    this._add({node, setBy, state, incNumber})
  }

  addAliveUpdate(node, setBy, incNumber) {
    this._add({node, setBy, state: ALIVE, incNumber})
  }

  addFaultyUpdate(node, setBy, incNumber) {
    this._add({node, setBy, state: FAULTY, incNumber})
  }

  addSuspectUpdate(node, setBy, incNumber) {
    this._add({node, setBy, state: SUSPECT, incNumber})
  }
  ...
  processAlive(update) {
    ...
    if (!this.opts.members.findMember(node)) {
      this.opts.members.addMember(node, state, setBy, incNumber)
    }
    // TODO

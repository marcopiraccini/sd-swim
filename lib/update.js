...
const {sortBy, remove, find, values, pull} = require('lodash')
...
class Update {
  ...
  addUpdate(node, setBy, state, incNumber) {
    ...
    const update = find(this._updates,
      el => (el.node.host === node.host && el.node.port === node.port && el.incNumber === incNumber))
    if (!update) {
    ...
    if (state === FAULTY) {
      ...
      if (!this._faulties.includes(key)) {
        this._faulties.push(key)
        setTimeout(() => {
          pull(this._faulties, key)
        }, 2000) // TODO: EXTERNALIZE, OR CALCULATE FROM SUSPECT-TIMEOUT
      }
    }
  }
  ...
  _processAlive(update) {
    ...
    // I propagate the message if I'm not the original source of th update. Otherwise message loop...
    if (!this.opts.members.isMe(setBy)) {
  ...
  _processSuspect(update) {
    ...
    // I propagate the message if I'm not the original source of the update. Otherwise message loop...
    if (!this.opts.members.isMe(setBy)) {
      this.propagate(update)
    }

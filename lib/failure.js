...
class Failure extends EventEmitter {
  ...
  _ping() {
    ...
    this.logger.debug(`Member selected for ping: ${targetNode}`)
 ...
 /**
  * Trigger the ping-req and set a timout. If this timeout expires, the node is
  * then
  */
  _pingReq(target) {
    this.logger.debug(`ping-req to ${target}`)
     ...
     this.pingReqTimers[key] = (function pingTimeout() {
       delete this.pingReqTimers[key]
       this.update.updateToSuspect(target) // NO ACK RECEIVED YET => UPDATE TO SUSPECT
     }, this.pingReqTimeout)
     for (const {node} of pingableMembers) {
       this.net.sendPingReq(node, target, this.sdswim.me)
     }
  ...
  pingReqReceived({sender: {host, port}}, msg) {
    this.logger.debug(`Received PING-REQ message from ${host}:${port} of type: ${msg.type} for request ${msg.request}`)
  ...
  ackReceived({sender}, msg) {
    this.logger.debug(`Received ACK message from ${sender.host}:${sender.port} of type: ${msg.type} with request ${msg.request}`)
    ...
    if (this.pingTimers[key]) {
      ...
      this.update.updateToAlive(sender)
      return
    }

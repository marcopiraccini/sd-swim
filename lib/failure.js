...
class Failure extends EventEmitter {
  ...
  */
  ...
  pingReceived({sender}, msg) {
    ...
    this.net.sendAck(sender)
  }

  pingReqReceived({sender: {host, port}}, msg) {
    this.logger.debug(`Received PING-REQ message from ${host}:${port} of type: ${msg.type}`)
    // TODO: Relay the ping
  }

  ackReceived({sender}, msg) {
    this.logger.debug(`Received ACK message from ${sender.host}:${sender.port} of type: ${msg.type}`)
    ...
    if (this.pingTimers[key]) {
      ...
      // Ack received, the node must be updated to "ALIVE"

...
class SDSwim extends EventEmitter {
  constructor (opts = {}) {
    ...
    // port
    if (opts.port === 0 || !opts.port) {  // default to 0
      // 0 is a valid value (use random port)
      this.port = 0
    }
    this.port = opts.port
    ...
    this.opts.pingReqGroupSize = this.pingReqGroupSize =
      opts.pingReqGroupSize || 3
    ...
    this.opts.disseminationFactor = this.disseminationFactor =
      opts.disseminationFactor || 15
    ...
    this.net.on('unknown', (sender, msg) =>
  ...
  get me () {
    return { host: this.host || '127.0.0.1', port: this.port }
  }

  whoami () {
    return Object.assign(this.me, {state: this.state})
  }
  ...
  // METADATA API. entries must be in the form [{key, value}]
  set entries (entries) {
    this.metadata.my = entries
  }

  // Get all the data, including mine.
  get data () {

...
function SDSwim(opts) {
  ...
  this.server.on('message', (sender, msg) => this.processMessage(sender, msg))
}
...
SDSwim.prototype.processMessage = function processMessage({sender: {host, port}}, msg) {
...
SDSwim.prototype.join = function join() {
  ...
  if (this.initialHosts) {
    this.client.sendJoin(this.initialHosts)
  }
}

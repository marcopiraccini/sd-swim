...
class SDSwim extends EventEmitter {
  constructor(opts) {
    ...
    // member list
    this._members = [] // TODO: Evaluate if to use a Set
   ...
   */
  _updateMember(node = {host: this.host, port: this.port}, state = this.nodeStates.ALIVE, setBy = {host: this.host, port: this.port}) {
    const current = find(this._members, el => (el.node.host === node.host  && el.node.port === node.port))
   ...
   */
  _getOtherMembers(membersToSkip = []) {
    membersToSkip.push({host: this.host, port: this.port}) // Esclude myself'node'
    // OK, I have to find a better way for a diff (maybe use Set?)
   return differenceBy(this._members, membersToSkip.map(el => ({node: el})), ({node: {host, port}}) => (`${host}:${port}`))
  }

...
class Messages {
  ...
  pingReqMessage (destination, updates) {
    const {host, port} = destination
    const data = encode({destination, type: MessageType.PING_REQ, updates})
    return ({host, port, type: MessageType.PING_REQ, data})
  }

  ackMessage (destination, updates) {
    const {host, port} = destination
    const data = encode({destination, type: MessageType.ACK, updates})

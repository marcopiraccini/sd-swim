...
class Messages {
  ...
  joinAckMessage (destination, initialMemberList) {
    ...
    const data = encode({destination, members: initialMemberList, type: MessageType.JOIN_ACK})
  ...
  metaMessage (destination, metadata) {
    const {host, port} = destination
    const data = encode({destination, type: MessageType.META, metadata})
    return ({host, port, type: MessageType.META, data})
  }
